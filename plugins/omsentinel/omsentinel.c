...
static rsRetVal ATTR_NONNULL(1) setPostURL(wrkrInstanceData_t *const pWrkrData) {
    ...
    if (url == NULL) {
        LogError(0, RS_RET_OUT_OF_MEMORY, "omsentinel: error allocating new estr for POST url.");
        ABORT_FINALIZE(RS_RET_ERR);
    }
    ...
    if (r != 0) {
        LogError(0, RS_RET_ERR,
                 "omsentinel: failure in creating restURL, "
                 "error code: %d",
                 r);
        ABORT_FINALIZE(RS_RET_ERR);
    }
...
static rsRetVal ATTR_NONNULL()
    ...
    if (pData->errorFile == NULL) {
        DBGPRINTF(
            "omsentinel: no local error logger defined - "
    ...
    if (pData->fdErrFile == -1) {
        ...
        if (pData->fdErrFile == -1) {
            LogError(errno, RS_RET_ERR, "omsentinel: error opening error file %s", pData->errorFile);
            ABORT_FINALIZE(RS_RET_ERR);
        }
    }
    ...
    if (wrRet != (ssize_t)toWrite) {
        LogError(errno, RS_RET_IO_ERROR, "omsentinel: error writing error file %s, write returned %lld", pData->errorFile,
                 (long long)wrRet);
    }
...
static rsRetVal checkResult(wrkrInstanceData_t *pWrkrData, uchar *reqmsg) {
    ...
    if (iRet != RS_RET_OK) {
        LogMsg(0, iRet, LOG_ERR, "omsentinel: checkResult error http status code: %ld reply: %s", statusCode,
...
static rsRetVal ATTR_NONNULL(1, 2) curlPost(
    wrkrInstanceData_t *pWrkrData, uchar *message, int msglen, const int nmsgs __attribute__((unused))) {
    CURLcode curlCode;
    ...
    if (curlCode != CURLE_OK) {
        ...
        LogError(0, RS_RET_SUSPENDED, "omsentinel: suspending ourselves due to server failure %lld: %s",
...
static rsRetVal serializeBatchJsonArray(wrkrInstanceData_t *pWrkrData, char **batchBuf) {
    ...
    for (size_t i = 0; i < numMessages; i++) {
        ...
        if (msgObj == NULL) {
            LogError(0, NO_ERRCODE, "omsentinel: serializeBatchJsonArray failed to parse %s as json, ignoring it",
...
static rsRetVal buildBatch(wrkrInstanceData_t *pWrkrData, uchar *message) {
    ...
    if (pWrkrData->batch.nmemb >= pWrkrData->pData->maxBatchSize) {
        LogError(0, RS_RET_ERR,
            "omsentinel: buildBatch something has gone wrong,"
...
BEGINbeginTransaction
    ...
    if (time(NULL) >= pData->authExp && pData->token) {
        if (pData->authReply) free(pData->authReply);
        if (pData->token) free(pData->token);
        if (pData->httpHeader) free(pData->httpHeader);
...
CODESTARTendTransaction
    ...
    if (pWrkrData->batch.nmemb > 0) {
        ...
        dbgprintf(
            "omsentinel: endTransaction, pWrkrData->batch.nmemb = 0, "
            "nothing to send. \n");
    }
finalize_it:
ENDendTransaction
...
static void ATTR_NONNULL(1) curlPostSetup(wrkrInstanceData_t *const pWrkrData) {
    ...
    if (cRet != CURLE_OK) DBGPRINTF("omsentinel: curlPostSetup unknown option CURLOPT_TCP_KEEPALIVE\n");
    /* keep-alive idle time to 120 seconds */
    cRet = curl_easy_setopt(pWrkrData->curlPostHandle, CURLOPT_TCP_KEEPIDLE, 120L);
    if (cRet != CURLE_OK) DBGPRINTF("omsentinel: curlPostSetup unknown option CURLOPT_TCP_KEEPIDLE\n");
    /* interval time between keep-alive probes: 60 seconds */
    cRet = curl_easy_setopt(pWrkrData->curlPostHandle, CURLOPT_TCP_KEEPINTVL, 60L);
    if (cRet != CURLE_OK) DBGPRINTF("omsentinel: curlPostSetup unknown option CURLOPT_TCP_KEEPINTVL\n");
}
        ...
        if (!strcmp(actpblk.descr[i].name, "dce")) {
            ...
            if (compressionLevel == -1 || (compressionLevel >= 0 && compressionLevel < 10)) {
                ...
                LogError(0, NO_ERRCODE,
                         "omsentinel: invalid compress.level %d using default instead,"
    ...
    for (inst = pModConf->root; inst != NULL; inst = inst->next) {
        ...
        if (inst->retryRulesetName) {
            ...
            if (localRet == RS_RET_NOT_FOUND) {
                LogError(0, localRet,
                         "omsentinel: retry.ruleset '%s' not found - "

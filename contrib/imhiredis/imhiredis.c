...
struct instanceConf_s {
	...
	prop_t *pInputName;		/* the input name property */
	uchar *pszInputName;		/* value of the intput name property */
...
static char const *REDIS_REPLIES[] = {
	"unknown", // 0
	"string", // 1
	"array", // 2
	"integer", // 3
...
#endif
rsRetVal redisConnectSync(redisContext **conn, redisNode *node, instanceConf_t *inst);
rsRetVal connectMasterSync(instanceConf_t *inst);
static sbool isConnectedSync(instanceConf_t *inst);
rsRetVal redisConnectAsync(redisAsyncContext **aconn, redisNode *node, instanceConf_t *inst);
...
checkInstance(instanceConf_t *const inst)
{
	...
	if (inst->redisNodesList->server != NULL && inst->redisNodesList->socketPath != NULL) {
		LogMsg(0, RS_RET_CONFIG_ERROR, LOG_WARNING,"imhiredis[%s]: both 'server' and 'socketPath' are given, "
								"ignoring 'socketPath'.", inst->pszInputName);
	...
	if(inst->redisNodesList->server != NULL && inst->redisNodesList->server[0] != '\0') {
		if (inst->redisNodesList->port == 0) {
			LogMsg(0, RS_RET_OK_WARN, LOG_WARNING, "imhiredis[%s]: port not set, setting default 6379",
				inst->pszInputName);
			inst->redisNodesList->port = 6379;
		}
		LogMsg(0, RS_RET_OK, LOG_INFO, "imhiredis[%s]: will use %s (%d) to connect",
			inst->pszInputName,
		...
		LogMsg(0, RS_RET_OK, LOG_INFO, "imhiredis[%s]: will use %s to connect",
			inst->pszInputName,
			inst->redisNodesList->socketPath);
		inst->redisNodesList->usesSocket = 1;
	} else {
		LogError(0, RS_RET_CONFIG_ERROR, "imhiredis[%s]: neither 'server' nor 'socketPath' are defined!",
			inst->pszInputName);
	...
	if (inst->use_tls) {
		if((inst->client_cert == NULL) ^ (inst->client_key == NULL)){
			LogMsg(0, RS_RET_CONFIG_ERROR, LOG_ERR, "imhiredis[%s]: \"client_cert\" and \"client_key\""
				" must be specified together!",
				inst->pszInputName);
			ABORT_FINALIZE(RS_RET_CONFIG_ERROR);
		}

		inst->ssl_conn = redisCreateSSLContext(
			inst->ca_cert_bundle,
			inst->ca_cert_dir,
			inst->client_cert,
			inst->client_key,
		...
		if (!inst->ssl_conn || ssl_error != REDIS_SSL_CTX_NONE) {
			LogError(0, RS_RET_REDIS_ERROR, "imhiredis[%s]: TLS configuration Error: %s",
				inst->pszInputName,
	...
	if (inst->mode < IMHIREDIS_MODE_QUEUE || inst->mode > IMHIREDIS_MODE_STREAM) {
		LogError(0, RS_RET_CONFIG_ERROR, "imhiredis[%s]: invalid mode, please choose 'subscribe', "
	...
	if (inst->mode != IMHIREDIS_MODE_QUEUE && inst->useLPop) {
		LogMsg(0, RS_RET_CONFIG_ERROR, LOG_WARNING,"imhiredis[%s]: 'uselpop' set with mode != queue : ignored.",
			inst->pszInputName);
	}

	if (inst->mode == IMHIREDIS_MODE_STREAM) {
		if(inst->streamConsumerGroup != NULL && inst->streamConsumerName == NULL) {
			LogError(0, RS_RET_CONFIG_ERROR, "imhiredis[%s]: invalid configuration, "
		...
		if(inst->streamAutoclaimIdleTime != 0 && inst->streamConsumerGroup == NULL) {
			LogMsg(0, RS_RET_CONFIG_ERROR, LOG_WARNING,"imhiredis[%s]: 'stream.autoclaimIdleTime' "
		...
		if (inst->streamConsumerGroup != NULL) {
			LogMsg(0, RS_RET_CONFIG_ERROR, LOG_WARNING,"imhiredis[%s]: 'stream.consumerGroup' "
									"set with mode != stream : ignored.",
									inst->pszInputName);
		}
		if (inst->streamConsumerName != NULL) {
			LogMsg(0, RS_RET_CONFIG_ERROR, LOG_WARNING,"imhiredis[%s]: 'stream.consumerName' "
									"set with mode != stream : ignored.",
									inst->pszInputName);
		}
		...
		if (inst->streamConsumerACK == 0) {
			LogMsg(0, RS_RET_CONFIG_ERROR, LOG_WARNING,"imhiredis[%s]: 'stream.consumerACK' "
									"disabled with mode != stream : ignored.",
									inst->pszInputName);
		}
		if (inst->fieldList.nmemb > 0) {
			LogMsg(0, RS_RET_CONFIG_ERROR, LOG_WARNING,"imhiredis[%s]: 'fields' "
	...
	if (inst->batchsize !=0 ) {
		LogMsg(0, RS_RET_OK, LOG_INFO,
			"imhiredis[%s]: batchsize is '%d'", inst->pszInputName, inst->batchsize);
	}
	else {
		LogMsg(0, RS_RET_OK_WARN, LOG_WARNING,
			"imhiredis[%s]: batchsize not set, setting to default (%d)",inst->pszInputName, BATCH_SIZE);
	...
	if (RS_RET_OK != redisActualizeCurrentNode(inst) || inst->currentNode == NULL) {
		LogMsg(0, RS_RET_REDIS_ERROR, LOG_WARNING, "imhiredis[%s]: could not find a valid master!",
			inst->pszInputName);
	}
...
{
	LogError(0, NO_ERRCODE, "imhiredis[%s]: ruleset '%s' not found - "
...
CODESTARTnewInpInst
	...
	if (inst->modeDescription == NULL) {
		...
		inst->mode = IMHIREDIS_MODE_SUBSCRIBE;
		LogMsg(0, RS_RET_OK_WARN, LOG_WARNING, "imhiredis[%s]: \"mode\" parameter not specified "
			"using default redis 'subscribe' mode -- this may not be what you want!",
			inst->pszInputName);
	}
	if (inst->key == NULL) {
		LogMsg(0, RS_RET_PARAM_ERROR, LOG_ERR, "imhiredis[%s]: \"key\" required parameter not specified!",
			inst->pszInputName);
		ABORT_FINALIZE(RS_RET_PARAM_ERROR);
	}
	if(inst->redisNodesList->server == NULL && inst->redisNodesList->socketPath == NULL) {
		...
		inst->redisNodesList->port = 6379;
		LogMsg(0, RS_RET_OK_WARN, LOG_WARNING, "imhiredis[%s]: no server parameter specified "
			"using default 127.0.0.1:6379 -- this may not be what you want!",
			inst->pszInputName);
	}
	if (inst->password == NULL) {
		LogMsg(0, RS_RET_OK, LOG_INFO, "imhiredis[%s]: no password specified",
			inst->pszInputName);
	}

	DBGPRINTF("imhiredis[%s]: newInpInst key=%s, mode=%s, uselpop=%d\n",
		inst->pszInputName, inst->key, inst->modeDescription, inst->useLPop);
...
CODESTARTrunInput
	...
	LogMsg(0, RS_RET_OK, LOG_DEBUG, "imhiredis: Starting %d imhiredis threads", activeHiredisworkers);
	...
	*/
	...
	LogMsg(0, RS_RET_OK, LOG_DEBUG, "imhiredis: terminating upon request of rsyslog core");
...
static void redisAsyncRecvCallback (redisAsyncContext *aconn, void *reply, void __attribute__((unused)) *unused) {
	...
	*/
	instanceConf_t *const inst = (instanceConf_t *)aconn->c.privdata;
 ...
 */
static void redisAsyncConnectCallback (const redisAsyncContext *ac, int status) {
	instanceConf_t *inst = (instanceConf_t *)ac->c.privdata;
	if (status != REDIS_OK) {
		LogMsg(0, RS_RET_REDIS_ERROR, LOG_ERR, "imhiredis[%s]: could not connect to redis (async): %s",
			inst->pszInputName,
			ac->errstr);
	...
	LogMsg(0, RS_RET_OK, LOG_DEBUG, "imhiredis[%s]: successfully connected (async)!", inst->pszInputName);
 ...
 */
static void redisAsyncDisconnectCallback (const redisAsyncContext *ac, int status) {
	...
	if (status != REDIS_OK) {
		LogMsg(0, RS_RET_REDIS_ERROR, LOG_ERR, "imhiredis[%s]: got disconnected from redis (async): %s",
			inst->pszInputName,
			ac->errstr);
		return;
	}
	LogMsg(0, RS_RET_OK, LOG_DEBUG, "imhiredis[%s]: successfully disconnected (async)!",
		inst->pszInputName);
 ...
 */
redisReply *getRole(redisContext *c) {
	instanceConf_t *inst = (instanceConf_t *)c->privdata;
	redisReply *reply;
	...
	if (reply == NULL) {
		LogMsg(0, RS_RET_REDIS_ERROR, LOG_WARNING, "imhiredis[%s]: could not get reply from ROLE command: %s",
			inst->pszInputName,
			c->errstr);
	}
	else if (reply->type == REDIS_REPLY_ERROR) {
		LogMsg(0, RS_RET_REDIS_ERROR, LOG_WARNING, "imhiredis[%s]: got an error while querying role -> %s",
			inst->pszInputName,
			reply->str);
		...
		LogMsg(0, RS_RET_REDIS_ERROR, LOG_ERR, "imhiredis[%s]: did not get an array from ROLE command",
			inst->pszInputName);
		freeReplyObject(reply);
		reply = NULL;
	}
...
static rsRetVal enqMsgJson(instanceConf_t *const inst, struct json_object *json, struct json_object *metadata) {
	...
	if(RS_RET_OK != MsgSetFlowControlType(pMsg, eFLOWCTL_LIGHT_DELAY))
		LogMsg(0, RS_RET_OK_WARN, LOG_WARNING,
			"imhiredis[%s]: Could not set Flow Control on message.", inst->pszInputName);
	if(inst->fieldList.nmemb != 0) {
		for (int i = 0; i < inst->fieldList.nmemb; i++)
		{
			...
			if(RS_RET_OK != msgAddJSON(pMsg, (uchar *)inst->fieldList.varname[i], tempJson, 0, 0)) {
				LogMsg(0, RS_RET_OBJ_CREATION_FAILED, LOG_ERR,
					"imhiredis[%s]: Failed to add value to '%s'",
					inst->pszInputName,
					inst->fieldList.varname[i]);
			}
		...
		if(RS_RET_OK != msgAddJSON(pMsg, (uchar*)"!", json, 0, 0)) {
			LogMsg(0, RS_RET_OBJ_CREATION_FAILED, LOG_ERR,
				"imhiredis[%s]: Failed to add json info to message!", inst->pszInputName);
			ABORT_FINALIZE(RS_RET_OBJ_CREATION_FAILED);
		}
	}
	if (metadata != NULL && RS_RET_OK != msgAddJSON(pMsg, (uchar*)".", metadata, 0, 0)) {
		LogMsg(0, RS_RET_OBJ_CREATION_FAILED, LOG_ERR,
			"imhiredis[%s]: Failed to add metadata to message!", inst->pszInputName);
		ABORT_FINALIZE(RS_RET_OBJ_CREATION_FAILED);
	}
	if(RS_RET_OK != submitMsg2(pMsg)) {
		LogMsg(0, RS_RET_OBJ_CREATION_FAILED, LOG_ERR,
			"imhiredis[%s]: Failed to submit message to main queue!", inst->pszInputName);
		ABORT_FINALIZE(RS_RET_OBJ_CREATION_FAILED);
	}
...
rsRetVal redisAuthentSynchronous(redisContext *conn, uchar *password) {
	...
	instanceConf_t *inst = (instanceConf_t *)conn->privdata;

	reply = (redisReply *) redisCommand(conn, "AUTH %s", password);
	if (reply == NULL) {
		LogError(0, RS_RET_REDIS_ERROR, "imhiredis[%s]: Could not authenticate!", inst->pszInputName);
		ABORT_FINALIZE(RS_RET_REDIS_ERROR);
	} else if (strncmp(reply->str, "OK", 2)) {
		LogError(0, RS_RET_REDIS_AUTH_FAILED, "imhiredis[%s]: Authentication failure -> %s",
			inst->pszInputName,
			reply->str);
...
rsRetVal redisAuthentAsynchronous(redisAsyncContext *aconn, uchar *password) {
	...
	instanceConf_t *inst = (instanceConf_t *)aconn->c.privdata;

	if (REDIS_OK != redisAsyncCommand(aconn, NULL, NULL, "AUTH %s", password)) {
		LogError(0, RS_RET_REDIS_ERROR, "imhiredis[%s]: error while authenticating asynchronously -> %s",
			inst->pszInputName,
			aconn->errstr);
		ABORT_FINALIZE(RS_RET_REDIS_ERROR);
	}
...
rsRetVal redisGetServersList(redisNode *node, instanceConf_t *inst, redisNode **result) {
	...
	if(reply == NULL) {
		LogMsg(0, RS_RET_REDIS_ERROR, LOG_WARNING, "imhiredis[%s]: did not get the role of the server",
			inst->pszInputName);
		ABORT_FINALIZE(RS_RET_REDIS_ERROR);
	}
...
rsRetVal redisAuthenticate(instanceConf_t *inst) {
	...
	if (inst->mode == IMHIREDIS_MODE_SUBSCRIBE) {
		if (RS_RET_OK != redisConnectSync(&usedContext, inst->currentNode, inst)) {
			LogMsg(0, RS_RET_REDIS_ERROR, LOG_WARNING, "imhiredis[%s]: could not connect to current "
...
rsRetVal redisInitSSLContext(redisContext *conn, redisSSLContext *ssl_context) {
	...
	if (redisInitiateSSLWithContext(conn, ssl_context) != REDIS_OK) {
		LogError(0, RS_RET_REDIS_ERROR, "imhiredis[%s]: error while initializing TLs context: %s",
			inst->pszInputName,
			conn->errstr);
		ABORT_FINALIZE(RS_RET_REDIS_ERROR);
	}
...
rsRetVal redisConnectSync(redisContext **conn, redisNode *node, instanceConf_t *inst) {
	...
	REDIS_OPTIONS_SET_PRIVDATA(&options, (void *)inst, NULL);
	...
	*conn = redisConnectWithOptions(&options);
	...
	if (*conn == NULL) {
		if (node->usesSocket) {
			LogError(0, RS_RET_REDIS_ERROR, "imhiredis[%s]: can not connect to redis server '%s' "
...
rsRetVal redisConnectAsync(redisAsyncContext **aconn, redisNode *node, instanceConf_t *inst) {
	...
	*aconn = redisAsyncConnectWithOptions(&options);
	...
	if(*aconn == NULL) {
		LogError(0, RS_RET_REDIS_ERROR, "imhiredis[%s]: could not allocate context (async)!",
			inst->pszInputName);
		ABORT_FINALIZE(RS_RET_REDIS_ERROR);
	} else if ((*aconn)->err) {
		if (node->usesSocket) {
			LogError(0, RS_RET_REDIS_ERROR, "imhiredis[%s]: cannot connect to server (async) '%s' -> %s",
				inst->pszInputName,
				node->socketPath,
				(*aconn)->errstr);
		} else {
			LogError(0, RS_RET_REDIS_ERROR, "imhiredis[%s]: cannot connect to server '%s', port '%d'"
				" (async) -> %s",
				inst->pszInputName,
				node->server,
				node->port,
...
rsRetVal connectMasterAsync(instanceConf_t *inst) {
	...
	if(RS_RET_OK != redisConnectAsync(&(inst->aconn), inst->currentNode, inst)) {
		inst->currentNode = NULL;
		ABORT_FINALIZE(RS_RET_REDIS_ERROR);
	}
	...
	// finalize async context creation
	...
	do {
		...
		do {
			if (REDIS_OK != redisGetReply(inst->conn, (void **) &reply)) {
				...
				LogError(0, RS_RET_REDIS_ERROR, "imhiredis[%s]: Error reading reply after POP #%d "
								"on key '%s' -> %s",
								inst->pszInputName,
								(inst->batchsize - i),
								inst->key,
				...
				if (reply != NULL) {
					...
					switch(replyType) {
						...
						case REDIS_REPLY_ERROR:
							...
							LogMsg(0, RS_RET_REDIS_ERROR, LOG_ERR, "imhiredis[%s]: error "
							...
							ABORT_FINALIZE(RS_RET_REDIS_ERROR);
						default:
							LogMsg(0, RS_RET_OK_WARN, LOG_WARNING, "imhiredis[%s]: "
					...
					LogMsg(0, RS_RET_REDIS_ERROR, LOG_ERR, "imhiredis[%s]: unexpected empty reply "
		...
		switch(reply->type) {
			...
			case REDIS_REPLY_ERROR:
				if(strcasestr(reply->str, "BUSYGROUP") != NULL) {
					...
					LogError(0, RS_RET_ERR, "imhiredis[%s]: An unknown error "
				...
				break;
			default:
				LogError(0, RS_RET_ERR, "imhiredis[%s]: An unknown reply was received "
					"while creating a consumer group -> %s",
					inst->pszInputName,
					REDIS_REPLIES[(reply->type)%15]);
				ABORT_FINALIZE(RS_RET_ERR);
		}
	} else {
		LogError(0, RS_RET_REDIS_ERROR, "imhiredis[%s]: Could not create group %s on stream %s!",
			inst->pszInputName,
			inst->streamConsumerGroup,
			inst->key);
		ABORT_FINALIZE(RS_RET_REDIS_ERROR);
	...
	if(reply != NULL) {
		switch(reply->type) {
			...
			case REDIS_REPLY_ERROR:
				...
				ABORT_FINALIZE(RS_RET_ERR);
			default:
				LogError(0, RS_RET_ERR, "imhiredis[%s]: unexpected reply type "
					"while aknowledging message: %s",
					inst->pszInputName,
					REDIS_REPLIES[(reply->type)%15]);
				ABORT_FINALIZE(RS_RET_ERR);
		}
		...
		LogError(0, RS_RET_REDIS_ERROR, "imhiredis[%s]: Could not ACK message with index %s for %s[%s]!",
			inst->pszInputName,
			index,
			stream,
			group);
...
static rsRetVal handleRedisXREADReply(instanceConf_t *const inst, const redisReply *reply) {
	...
	if(reply == NULL || reply->type != REDIS_REPLY_ARRAY) {
		...
		LogMsg(0, RS_RET_OK_WARN, LOG_WARNING, "imhiredis[%s]: Reply is not an array, ignoring",
			inst->pszInputName);
		...
		for(size_t i = 0; i < reply->elements; i++) {
			...
			if(streamObj->type != REDIS_REPLY_ARRAY || streamObj->elements != 2) {
				LogMsg(0, RS_RET_OK_WARN, LOG_WARNING, "imhiredis[%s]: wrong object format, "
					"object should contain the name of the stream and an array of messages",
					inst->pszInputName);
				ABORT_FINALIZE(RS_RET_OK_WARN);
			}
			if(streamObj->element[0]->type != REDIS_REPLY_STRING) {
				LogMsg(0, RS_RET_OK_WARN, LOG_WARNING, "imhiredis[%s]: wrong field format, "
			...
			for(size_t j = 0; j < msgList->elements; j++) {
				...
				if(msgObj->type != REDIS_REPLY_ARRAY || msgObj->elements != 2) {
					LogMsg(0, RS_RET_OK_WARN, LOG_WARNING, "imhiredis[%s]: wrong object "
						"format, object should contain the index and its content(s)",
						inst->pszInputName);
					ABORT_FINALIZE(RS_RET_OK_WARN);
				}
				if(msgObj->element[0]->type != REDIS_REPLY_STRING) {
					LogMsg(0, RS_RET_OK_WARN, LOG_WARNING, "imhiredis[%s]: wrong field "
...
static rsRetVal handleRedisXAUTOCLAIMReply(
	...
	if(reply == NULL || reply->type != REDIS_REPLY_ARRAY) {
		...
		if(reply->elements < 2 || reply->elements > 3) {
			LogMsg(0, RS_RET_OK_WARN, LOG_WARNING, "imhiredis[%s]: wrong number of fields, "
				"cannot process entry",
				inst->pszInputName);
			ABORT_FINALIZE(RS_RET_OK_WARN);
		}
		if(reply->element[0]->type != REDIS_REPLY_STRING) {
			LogMsg(0, RS_RET_OK_WARN, LOG_WARNING, "imhiredis[%s]: the first element "
		...
		if(msgList->type != REDIS_REPLY_ARRAY) {
			LogMsg(0, RS_RET_OK_WARN, LOG_WARNING, "imhiredis[%s]: the second element "
		...
		for(size_t j = 0; j < msgList->elements; j++) {
			...
			// Object should contain the name of the index, and its content(s)
			if(msgObj->type != REDIS_REPLY_ARRAY || msgObj->elements != 2) {
				LogMsg(0, RS_RET_OK_WARN, LOG_WARNING, "imhiredis[%s]: wrong message "
					"format, cannot process",
					inst->pszInputName);
				ABORT_FINALIZE(RS_RET_OK_WARN);
			}
			if(msgObj->element[0]->type != REDIS_REPLY_STRING) {
				LogMsg(0, RS_RET_OK_WARN, LOG_WARNING, "imhiredis[%s]: first message "
			...
			if(msgObj->type != REDIS_REPLY_ARRAY) {
				LogMsg(0, RS_RET_OK_WARN, LOG_WARNING, "imhiredis[%s]: second message "
...
rsRetVal redisStreamRead(instanceConf_t *inst) {
	...
	if(inst->streamAutoclaimIdleTime != 0) {
		...
		if(inst->streamReadFrom[0] == '$') {
			LogMsg(0, RS_RET_OK, LOG_WARNING, "imhiredis[%s]: Cannot claim pending entries from '$', "
	...
	do {
		...
		if(reply == NULL) {
			LogError(0, RS_RET_REDIS_ERROR, "imhiredis[%s]: Error while trying to read stream '%s'",
				inst->pszInputName,
				inst->key);
			ABORT_FINALIZE(RS_RET_REDIS_ERROR);
		}
		...
		switch(replyType) {
			...
			case REDIS_REPLY_ERROR:
				...
				ABORT_FINALIZE(RS_RET_REDIS_ERROR);
			default:
				LogMsg(0, RS_RET_OK_WARN, LOG_WARNING, "imhiredis[%s]: unexpected "
					"reply type: %s",
					inst->pszInputName,
					REDIS_REPLIES[replyType%15]);
		}
...
rsRetVal redisSubscribe(instanceConf_t *inst) {
	...
	if (ret != REDIS_OK) {
		LogMsg(0, RS_RET_REDIS_ERROR, LOG_ERR, "imhiredis[%s]: Could not subscribe", inst->pszInputName);
		ABORT_FINALIZE(RS_RET_REDIS_ERROR);
	}
...
void workerLoop(struct imhiredisWrkrInfo_s *me) {
	...
	while(glbl.GetGlobalInputTermState() == 0) {
		if (!me->fnIsConnected(me->inst)) {
			 ...
			 */
			LogMsg(0, RS_RET_OK, LOG_INFO, "imhiredis[%s]: "
			...
			if (me->inst->currentNode != NULL) {
				rsRetVal ret = me->fnConnectMaster(me->inst);
				if(ret != RS_RET_OK) {
					LogMsg(0, ret, LOG_WARNING, "imhiredis[%s]: "
